...
class UniADetZeroShot(nn.Module):
    def __init__(
        self,
        backbone: nn.Module,
    ...
    def forward(self, image: torch.Tensor):
        ...
        for layer, cls_token, patch_tokens in zip(
            ...
            map_score = anomaly_map.amax(dim=(1, 2))
            image_score = (1.0 - self.score_fusion_weight) * cls_score + self.score_fusion_weight * map_score

            seg_probs_per_layer.append(seg_probs)
            anomaly_maps_per_layer.append(anomaly_map)
            image_scores_per_layer.append(image_score)

        final_anomaly_map = torch.stack(anomaly_maps_per_layer, dim=0).mean(dim=0)
        final_image_score = torch.stack(image_scores_per_layer, dim=0).mean(dim=0)
        ...
        return {
            ...
            "image_scores_per_layer": image_scores_per_layer,
